...
export const codegenTargets = ['client', 'server', 'browser', 'all'] as const
const codegenTargetSet = new Set<string>(codegenTargets)
export type CodegenTarget = (typeof codegenTargets)[number]
...
export function isCodegenTarget(value: unknown): value is CodegenTarget {
  return typeof value === 'string' && codegenTargetSet.has(value)
}

export function parseCodegenTarget(raw: string): InvalidCodegenTargetError | CodegenTarget {
  if (isCodegenTarget(raw)) return raw
