...
describe('passport check handler', () => {
  ...
  it('ignores the forced recovery lever when it is off', async () => {
    const { context, evaluate, retainPageForRecovery } = makeContext({})
    const result = await makePassportCheckHandler({
      checkerUrl: 'http://fixture.local/solutions/checker',
      recoverySshTarget: 'pi@myboard.local',
    ...
    expect(result).toEqual({ status: 1, send_status_msg: 'ok' })
    expect(evaluate).toHaveBeenCalledTimes(2)
    expect(retainPageForRecovery).not.toHaveBeenCalled()
  })

  it('forces exactly one recovery on the first check and lets the second complete normally', async () => {
    const handler = makePassportCheckHandler({
      checkerUrl: 'http://fixture.local/solutions/checker',
      recoverySshTarget: 'pi@myboard.local',
      forceRecovery: true,
    })
    ...
    const forced = await handler({}, first.context)

    expect(forced).toBeInstanceOf(BrowserSessionRequiredError)
    expect((forced as BrowserSessionRequiredError).sshTarget).toBe('pi@myboard.local')
    expect(first.retainPageForRecovery).toHaveBeenCalledOnce()
    // The retained page must be the navigated checker page, so a human can drive
    // the site's own UI over noVNC: goto ran, and the single evaluate is the
    // navigation-evidence probe (called with no arg) — the passport submission,
    // which is the only evaluate that takes an arg, never ran.
    expect(first.goto).toHaveBeenCalledOnce()
    expect(first.evaluate).toHaveBeenCalledTimes(1)
    expect(first.evaluate.mock.calls[0]?.[1]).toBeUndefined()

    const second = makeContext({})
    const normal = await handler({}, second.context)

    expect(normal).toEqual({ status: 1, send_status_msg: 'ok' })
    expect(second.retainPageForRecovery).not.toHaveBeenCalled()
    expect(second.evaluate).toHaveBeenCalledTimes(2)
  })

  it('keeps the forced recovery latch armed when navigation fails', async () => {
    const handler = makePassportCheckHandler({
      checkerUrl: 'http://fixture.local/solutions/checker',
      recoverySshTarget: 'pi@myboard.local',
      forceRecovery: true,
    ...
    const failing = makeContext({ navigationError: new Error('navigation failed') })
    const navigationFailure = await handler({}, failing.context)

    expect(navigationFailure).toBeInstanceOf(UpstreamResponseError)
    expect(failing.retainPageForRecovery).not.toHaveBeenCalled()

    const retry = makeContext({})
    const forced = await handler({}, retry.context)

    expect(forced).toBeInstanceOf(BrowserSessionRequiredError)
    expect(retry.retainPageForRecovery).toHaveBeenCalledOnce()
  })

  it('gives every constructed handler its own forced recovery latch', async () => {
    const options = {
      checkerUrl: 'http://fixture.local/solutions/checker',
      recoverySshTarget: 'pi@myboard.local',
      forceRecovery: true,
    ...
    expect(await makePassportCheckHandler(options)({}, first.context)).toBeInstanceOf(
      BrowserSessionRequiredError,
    )
    expect(await makePassportCheckHandler(options)({}, second.context)).toBeInstanceOf(
      BrowserSessionRequiredError,
    )
  })
