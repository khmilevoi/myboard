...
Widget _internals_ (Clock, Ofelia) and the `WidgetMode = 'small' | 'large'` contract are unchanged.
...
- shadcn's own `--accent` token (a muted _hover_ background, distinct from the brand) maps to `--secondary`/`--muted`; the brand violet is `--primary`. Do not conflate them.
