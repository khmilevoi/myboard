...
5. **The dismiss-race mitigation is event isolation** (capture-phase Esc + stopPropagation of `pointerdown`/`focusin`), not the "ref cleared one tick late" pattern — the latter needs cooperation from the underlying dialog, which a widget cannot patch; isolation prevents the race from starting at all.
