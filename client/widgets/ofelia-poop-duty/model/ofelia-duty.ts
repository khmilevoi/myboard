...
export function latestOutcomesByDate(
  entries: LedgerEntry[],
...
export const ofeliaDutyModel = ({ storage, timer }: OfeliaDutyModelProps) => {
  ...
  const onLedgerChange = wrap(
    (event: StorageError | StorageChange<LedgerEntry[]>) => {
  ...
  const historyView = computed<HistoryEntryView[]>(() => {
    ...
    return entries
      .filter(
        (entry) =>
          weekStartISO(Temporal.PlainDate.from(entry.date)) === weekIso,
      )
  ...
  const forgivePending = computed(
    () => forgive.pending() > 0,
    "ofeliaDuty.forgivePending",
  );
